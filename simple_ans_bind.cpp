...
PYBIND11_MODULE(_simple_ans, m) {
    ...
    m.def("encode", [](py::array_t<int32_t> signal,
        ...
        if (counts_buf.shape[0] != values_buf.shape[0]) {
            throw std::runtime_error("symbol_counts and symbol_values must have the same length");
        }

        return simple_ans::encode(
            static_cast<const int32_t*>(signal_buf.ptr), signal_buf.size,
    ...
    m.def("decode", [](uint32_t state,
        ...
        simple_ans::decode(
            static_cast<int32_t*>(result_buf.ptr), n,
            state, bitstream.data(), num_bits,
            static_cast<const uint32_t*>(counts_buf.ptr),
            static_cast<const int32_t*>(values_buf.ptr),
            counts_buf.shape[0]
        );
    ...
    m.def("choose_symbol_counts", [](py::array_t<double> proportions, uint32_t L) {
        ...
        simple_ans::choose_symbol_counts(
            static_cast<uint32_t*>(result_buf.ptr),
       ...
       py::arg("proportions").noconvert(), py::arg("L"));

    m.def("add_one_test", [](py::array_t<int32_t> input) {
        py::buffer_info buf = input.request();
        auto result = py::array_t<int32_t>(buf.shape[0]);
        py::buffer_info result_buf = result.request();

        simple_ans::add_one_test(
            static_cast<int32_t*>(result_buf.ptr),
